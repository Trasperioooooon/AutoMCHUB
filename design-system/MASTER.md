...
- **稀疏页竖向居中**：`#main.center` 仅用于内容短、不会溢出的页面（创建入口、空态）。
...
| `.modal` / `.toast` | modal 460px（onboard 500px）+ mask 模糊；toast 右上、左侧 3px 强调线、错误=红线 | Esc 关、点遮罩关、焦点归还 |
| `.empty` 空态 | 虚线框 + 居中 + 主行动按钮 | 空态是行动邀请，不是死路（零状态即主行动） |
